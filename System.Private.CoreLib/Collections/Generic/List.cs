...
namespace System.Collections.Generic
{
    ...
    public class List<T> : IList<T>, IList, IReadOnlyList<T>
    {
        ...
        // TODO: use the sublength in the array instead, so we can avoid the double 
        //       length check 
...
#pragma warning restore CA1825
            ...
            set
            {
                ...
                if (value != _items.Length)
                {
                    if (value > 0)
                    {
                        ...
                        if (_size > 0)
                        {
                            _items.AsSpan(0, _size).CopyTo(newItems);
                        }
                        _items = newItems;
                    }
                    else
        ...
        public int RemoveAll(Predicate<T> match)
        {
            ...
            if (RuntimeHelpers.IsReferenceOrContainsReferences<T>())
            {
                _items.AsSpan(freeIndex, _size - freeIndex).Clear();
            }
        ...
        public void Reverse(int index, int count)
        {
            ...
            if (count > 1)
            {
                _items.AsSpan(index, count).Reverse();
            }
            _version++;
        }
        ...
        public void Sort(int index, int count, IComparer<T>? comparer)
        {
            ...
            if (count > 1)
            {
                _items.AsSpan(index, count).Sort(comparer);
            }
            _version++;
        }
        ...
        public T[] ToArray()
        {
            ...
            T[] array = new T[_size];
            _items.AsSpan(0, _size).CopyTo(array);
            return array;
        }
