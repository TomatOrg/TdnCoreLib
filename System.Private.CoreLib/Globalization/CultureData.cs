...
internal sealed partial class CultureData
{
    ...
    internal static CultureData? GetCultureDataForRegion(string? cultureName, bool useUserOverride)
    {
        ...
        if (retVal == null || retVal.IsNeutralCulture)
        {
            retVal = IcuGetCultureDataFromRegionName();
        }
...
#pragma warning disable 618
                                                    ...
                                                    CultureTypes.UserCustomCulture)) != 0)
        {
            throw new ArgumentOutOfRangeException(nameof(types),
                string.Format(SR.ArgumentOutOfRange_Range, CultureTypes.NeutralCultures, CultureTypes.UserCustomCulture));
        }
...
#pragma warning restore 618
    ...
    private static string NormalizeCultureName(string name, out bool isNeutralName)
    {
        ...
        if (name.Length > LocaleNameMaxLength)
        {
            ...
            throw new ArgumentException(string.Format(SR.Argument_InvalidId, nameof(name)));
        }
    ...
    /// </summary>
    internal string ThreeLetterWindowsLanguageName => _sAbbrevLang ??= 
    ...
    internal int GeoId
    {
        get
        {
            if (_iGeoId == undef)
            {
                _iGeoId = IcuGetGeoId(_sRealName!);
            }
            return _iGeoId;
        }
    }
    ...
    internal string LocalizedCountryName
    {
        get
        {
            ...
            if (localizedCountry == null)
            {
                try
                {
                    localizedCountry = IcuGetRegionDisplayName();
                }
                catch
                {
    ...
    /// </summary>
    internal string SCONSOLEFALLBACKNAME => _sConsoleFallbackName ??=
    ...
    /// (user can override)
    /// </summary>
    internal string ListSeparator => _sListSeparator ??= IcuGetListSeparator(_sWindowsName);
    ...
    internal int LCID
    {
        get
        {
            if (_iLanguage == 0)
            {
                ...
                _iLanguage = IcuLocaleNameToLCID(_sRealName);
            }
            return _iLanguage;
        }
    }
    ...
    internal void GetNFIValues(NumberFormatInfo nfi)
    {
        ...
        else
        {
            ...
            nfi._digitSubstitution = IcuGetDigitSubstitution(_sRealName);
        }
    ...
    internal static string AnsiToLower(string testString) => testString;
