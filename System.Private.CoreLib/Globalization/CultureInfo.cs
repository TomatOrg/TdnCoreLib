...
public partial class CultureInfo : IFormatProvider, ICloneable
{
    ...
    private static string GetCultureNotSupportedExceptionMessage() => SR.Argument_CultureNotSupported;
    ...
    internal static bool VerifyCultureName(string cultureName, bool throwException)
    {
        ...
        for (int i = 0; i < cultureName.Length; i++)
        {
            ...
            if (throwException)
            {
                throw new ArgumentException(string.Format(SR.Argument_InvalidResourceCultureName, cultureName));
            }
            return false;
        }
        return true;
...
#pragma warning disable CA2208 // Instantiate argument exceptions correctly, combination of arguments used
            throw new CultureNotFoundException("name/altName", string.Format(SR.Argument_OneOfCulturesNotSupported, name, altName));
#pragma warning restore CA2208
    ...
    public static CultureInfo GetCultureInfo(string name, bool predefinedOnly)
    {
        ...
        if (predefinedOnly)
        {
            if (!CultureData.IcuIsEnsurePredefinedLocaleName(name))
            {
                throw new CultureNotFoundException(nameof(name), name, string.Format(SR.Argument_InvalidPredefinedCultureName, name));
            }
        }
    ...
    public static CultureInfo GetCultureInfoByIetfLanguageTag(string name)
    {
        ...
        {
            throw new CultureNotFoundException(nameof(name), string.Format(SR.Argument_CultureIetfNotSupported, name));
        }
